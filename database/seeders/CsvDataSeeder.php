...
class CsvDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        //
        // Ruta al archivo CSV
        $csvFile = base_path('database/seeders/final_file.csv'); // Cambia esto a la ruta real de tu archivo CSV

        // Crear un reader para el CSV
        $csv = Reader::createFromPath($csvFile, 'r');
        $csv->setHeaderOffset(0); // Configurar el offset de encabezado para omitir la primera fila

        // Obtener todos los registros del CSV
        $records = $csv->getRecords();

        // Insertar cada registro en la tabla csv_data
